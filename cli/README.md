...
```bash
uvx --from geolens-cli==1.14.2 geolens validate cli/geolens.yaml
...
```bash
uvx --from geolens-cli==1.14.2 geolens --json apply --dry-run cli/geolens.yaml
```
...
```bash
uvx --from geolens-cli==1.14.2 geolens --json apply cli/geolens.yaml
```
...
```bash
echo "$GEOLENS_API_KEY" | uvx --from geolens-cli==1.14.2 geolens login \
...
GEOLENS_INSTANCE=https://demo.getgeolens.com \
  uvx --from geolens-cli==1.14.2 geolens apply --dry-run cli/geolens.yaml
