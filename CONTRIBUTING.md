...
`.github/workflows/verify.yml` runs on every pull request, on pushes to `main`, every morning on a schedule, and whenever `geolens-io/geolens` sends a `demo-reseeded` dispatch (asked for in geolens#2142). A scheduled or reseed run that goes red opens a `Scheduled verification is red` issue, and the next green one closes it:

- **Demo fixtures**: `ci/check-fixtures.mjs` runs first and probes the demo datasets, tile paths, exports and any share token named in `ci/fixtures.json`, which is where the IDs hardcoded across the examples are declared. The saved maps the gallery links by UUID are probed too, since no example opens them and the sweep would never notice one going missing. Red there means the demo changed under the examples, not that an example broke.
- **Browser examples**: `ci/verify-examples.mjs` loads every entry in `ci/manifest.json` with Playwright against the real demo. A page passes only if it reaches the demo, loads the specific data its manifest entry names, gets features back rather than an empty collection, keeps the console free of errors from the demo or the page itself, and actually paints. The middle of the viewport is screenshotted and must not be a flat fill. HTTP status alone is not the check: a vector tile answers 200 with its source-layer named wrong while the map stays blank. Failures from third-party hosts (a CDN, an analytics beacon) and cancelled requests are reported but never fail the build, and a page that fails is retried once before it counts as red. Pushes, pull requests and the daily schedule sweep in Chromium; the Monday schedule and manual runs sweep the whole manifest again in Firefox and WebKit (`BROWSERS=chromium,firefox,webkit`), and the job summary reports each engine on its own row.
- **Python examples**: runs `python/analyze.py` with `uv run` and checks it produces `subway.png`, then `python/sdk-catalog.py` and `duckdb/query.py` the same way. Separate steps, so a failure names the script rather than the job. `duckdb/query.py` needs no artifact check: it asserts its own row counts, the extent its coordinates project into, and the shape of its spatial join, and exits non-zero when one is wrong.
...
- **Outbound links**: `ci/check-links.py` collects every link to `docs.getgeolens.com`, `getgeolens.com` and a `github.com/geolens-io/...` file, directory, issue or pull request, and GETs each one. A docs or marketing link with a `#fragment` must also find `id="<fragment>"` on the page, so a renamed heading fails the same way a renamed route does. Either failure names the URL, what was wrong and the files that carry it. Links into this repo's own `main` are checked against the checkout instead, so a pull request that adds a file and links it is not failed for a page GitHub cannot serve until the merge.
- **Docs contract**: `ci/check-contract.py` fetches `https://getgeolens.com/docs-contract.json`, the facts every GeoLens surface is held to, and fails on any of its `forbidden` patterns or on an install line that is not exactly its one-liner. A contract it cannot fetch only warns.
...
`.github/workflows/qgis.yml` runs on the first day of each month and on demand. No hosted runner has QGIS, so it runs `qgis/verify.py` inside the upstream `qgis/qgis` image, once on 4.2 (the release `qgis/README.md` documents) and once on the 3.44 LTR, each pinned by digest, with `QT_QPA_PLATFORM=offscreen`: the two subway collections over OGC API - Features and the DEM as XYZ tiles have to load, count right and draw. It runs monthly because the providers it exercises change with QGIS releases rather than with commits to this repo.

`.github/workflows/release-dispatch.yml` runs on nothing you push. It listens for a `geolens-release` repository dispatch from `geolens-io/geolens`, sent when a GA release publishes there. When the release is ahead of the pins, it runs `ci/check-pins.py --bump` on the `chore/pins-auto` branch and opens a pull request for it with a checklist; a newer release before that one merges bumps on top of it and comments, so fixes already pushed there stay. Once PyPI and npm serve the new packages (it waits up to 30 minutes), it dispatches **Verify examples** on the branch, all three engines included. The workflow token cannot push changes under `.github/workflows`, so no pin may live there: `ci/check-pins.py` fails on one, and `verify.yml` reads its CLI version from `cli/github-actions.yml`. When the pins already match, it logs a notice and stops. A release is a task here rather than a notification: the demo these examples run against is redeployed from it, so the pins are stale the moment it ships. Opening the pull request needs "Allow GitHub Actions to create and approve pull requests", which the geolens-io organisation turns off. While it is off, the workflow opens one issue instead (refreshed by later releases) that links the branch, and a person opens the pull request from there.
