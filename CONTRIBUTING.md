...
- **DuckDB example**: a third step in the same Python job, `uv run duckdb/run.py`, executes `duckdb/features.sql` against the demo and fails if any `SELECT` comes back with no rows. A DuckDB failure therefore shows as "Python examples" red; the step name says which.
