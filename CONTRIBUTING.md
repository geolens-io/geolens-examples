...
`.github/workflows/release-dispatch.yml` runs on nothing you push. It listens for a `geolens-release` repository dispatch from `geolens-io/geolens`, sent when a GA release publishes there. When the release is ahead of the pins, it runs `ci/check-pins.py --bump` on the `chore/pins-auto` branch and opens a pull request for it with a checklist; a newer release before that one merges bumps on top of it and comments, so fixes already pushed there stay. Once PyPI and npm serve the new packages (it waits up to 30 minutes), it dispatches **Verify examples** on the branch, all three engines included. The workflow token cannot push changes under `.github/workflows`, so no pin may live there: `ci/check-pins.py` fails on one, and `verify.yml` reads its CLI version from `cli/github-actions.yml`. When the pins already match, it logs a notice and stops. A release is a task here rather than a notification: the demo these examples run against is redeployed from it, so the pins are stale the moment it ships. The workflow needs "Allow GitHub Actions to create and approve pull requests" turned on in the repository's Actions settings, since a pull request opened with the workflow token starts no workflows of its own and that setting is what lets it open one at all.
