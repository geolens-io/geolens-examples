...
- **Demo fixtures**: `ci/check-fixtures.mjs` runs first and probes the demo datasets, tile paths and share token named in `ci/fixtures.json`, which is where the IDs hardcoded across the examples are declared. The three saved maps the gallery links by UUID are probed too, since no example opens them and the sweep would never notice one going missing. Red there means the demo changed under the examples, not that an example broke.
